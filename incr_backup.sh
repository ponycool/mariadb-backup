...
echo "start exec $BACKUP_EX $BACKUP_OPTIONS --back --target-dir $TMP_INCR_DIR --incremental-basedir $INCR_BASE_DIR > $TMP_FILE 2>&1"

$BACKUP_EX $BACKUP_OPTIONS --back --target-dir "$TMP_INCR_DIR" --incremental-basedir "$INCR_BASE_DIR" >"$TMP_FILE" 2>&1
